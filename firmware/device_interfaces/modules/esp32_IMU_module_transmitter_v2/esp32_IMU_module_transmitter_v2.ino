/*  this is code for esp32 IMU BLE transmitter module - v2
 *  v2 adds BLEServerCallbacks so the module re-advertises after a
 *  disconnect (no more hardware reset between client connections).
...
void setup() {
  ...
  pServer->setCallbacks(new ServerCallbacks());                 // v2
